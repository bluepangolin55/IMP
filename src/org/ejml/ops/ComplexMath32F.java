...
public class ComplexMath32F
{
    ...
    public static void convert( ComplexPolar32F input , Complex32F output ) {
        output.real = (float) (input.r*Math.cos(input.theta));
        output.imaginary = (float) (input.r*Math.sin(input.theta));
    }
    ...
    {
        result.r = (float) Math.pow(a.r,1.0/N);
        result.theta = (float) ((a.theta + 2.0*k*Math.PI)/N);
    }
    ...
    public static void root( Complex32F a , int N , int k , Complex32F result )
    {
        ...
        r = (float) Math.pow(r,1.0/N);
        theta = (float) ((theta + 2.0*k*Math.PI)/N);

        result.real = (float) (r*Math.cos(theta));
        result.imaginary = (float) (r*Math.sin(theta));
    }
    ...
    public static void sqrt(Complex32F input, Complex32F root)
    {
        ...
        root.real = (float) Math.sqrt((r+a)/2.0);
        root.imaginary = (float) Math.sqrt((r-a)/2.0);
